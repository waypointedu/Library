    ...
    <div>
      ...
      <div className="mb-4">
        <Textarea
          ...
          className="mb-2"
        />
        <Button onClick={handleSubmit} disabled={!newComment.trim() || createComment.isPending} size="sm">
          <Send className="w-4 h-4 mr-1" />{t.post}
        </Button>
      </div>
          ...
          <Card key={comment.id}>
            <CardContent className="p-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs text-slate-400">{format(new Date(comment.created_date), 'MMM d, yyyy')}</span>
              </div>
